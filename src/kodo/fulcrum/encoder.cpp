...
#include "../detail/fulcrum/stack_encoder.hpp"
...
{
    using stack_type = kodo::detail::fulcrum::stack_encoder<Math>;
...
finite_field encoder::inner_field() const
{
    assert(m_impl);
...
auto encoder::expansion() const -> uint64_t
{
    assert(m_impl);
    return m_impl->expansion().value;
}

auto encoder::inner_symbols() const -> uint64_t
{
    assert(m_impl);
...
auto encoder::rank() const -> uint64_t
{
    assert(m_impl);
    return m_impl->rank().value;
}

auto encoder::inner_rank() const -> uint64_t
{
    assert(m_impl);
