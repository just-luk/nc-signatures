...
int CodedPiece::len() {
    return piece.size() + codingVector.size();
}
...
std::vector<Fr> generateCodingVector(uint n) {
    std::vector<Fr> ret(n);
    for(int i = 0; i < n; i++) {
        ret[i].setRand();
    }
...
std::vector<std::vector<Fr>> OriginalPiecesWithCountAndSize(std::vector<unsigned char> data, uint pieceCount, uint pieceSize) {
    data.resize(pieceSize * pieceCount, 0);
    std::vector<std::vector<Fr>> ret(pieceCount);
    for(int i = 0; i < pieceCount; i++) {
        ret[i].resize(pieceSize);
        copy(data.begin() + i * pieceSize, data.begin() + (i + 1) * pieceSize, ret[i].begin());
    }
    return ret;
}

std::vector<std::vector<Fr>> OriginalPiecesFromDataAndPieceCount(std::vector<unsigned char> data, uint pieceCount) {
    uint size = data.size();
    uint pieceSize = ceil(size / pieceCount);
    return OriginalPiecesWithCountAndSize(data, pieceCount, pieceSize);
}

std::vector<std::vector<Fr>> OriginalPiecesFromDataAndPieceSize(std::vector<unsigned char> data, uint pieceSize) {
    uint size = data.size();
    uint pieceCount = ceil(size / pieceSize);
    return OriginalPiecesWithCountAndSize(data, pieceCount, pieceSize);
}