...
bool Verify(const G1& sign, const G2& u, const G2& h, std::vector<Fr>& vector, const std::string& id)
{
	Fp12 e1, e2;
	G1 hashed;
...
int main() {
    ...
    uint pieceCount = 64;
    uint codedPieceCount = pieceCount * 2;
    uint droppedPieceCount = pieceCount / 2;

    FullRLNCEncoder encoder(fileData, pieceCount, "logo.png123", alpha, false);

    std::vector<CodedPiece> codedPieces;
    for(uint i = 0; i < codedPieceCount; i++) {
