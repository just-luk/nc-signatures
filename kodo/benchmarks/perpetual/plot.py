...
def plot_throughput(df: pd.DataFrame):
    ...
    df["block_bytes"] = df["block_bytes"].astype(int)
    df["symbol_bytes"] = df["symbol_bytes"].astype(int)
    df["outer_interval"] = df["outer_interval"].astype(int)
    df["outer_segments"] = df["outer_segments"].astype(int)
    df["throughput"] = df["throughput"].apply(bps_to_mbps)

    fixed_fields = ["benchmark", "symbol_bytes"]
    ...
    for key, group in op:
        ...
        chart = (
            ...
            .mark_point()
            .encode(
                x=alt.X("run_index:Q", axis=alt.Axis(title="Run [-]")),
        ...
        chart += (
            alt.Chart(group)
            .mark_rule()
            .encode(y="mean(throughput):Q", color="block_bytes:N")
        )
        charts.append(chart.to_json(indent=None))

    fixed_fields = ["block_bytes", "symbol_bytes"]

    op = df.groupby(fixed_fields)

    for key, group in op:

        title = "Block Encode vs Decode"
    ...
    chart_path = os.path.abspath(
        os.path.join(".", "benchmark_results/perpetual_chart.html")
    )
    ...
    print("Writing perpetual_chart.html...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="input",
        required=True,
        help="The input .json file produced by the " "kodo perpetual benchmark",
    )
