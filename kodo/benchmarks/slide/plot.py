...
def plot_throughput(df: pd.DataFrame):
    ...
    fixed_fields = ["benchmark", "symbols", "symbol_bytes"]
    ...
    fixed_fields = ["symbols", "symbol_bytes", "systematic"]

    op = df.groupby(fixed_fields)

    for key, group in op:

        title = "Slide Encode vs Decode"
    ...
    chart_path = os.path.abspath(
        os.path.join(".", "benchmark_results/slide_chart.html")
    )
    ...
    print("Writing slide_chart.html...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="input",
        required=True,
        help="The input .json file produced by the " "kodo slide benchmark",
    )
