...
#include "../detail/block/stack_decoder.hpp"
...
struct decoder::interface
{
    ...
    virtual auto coefficients_data(detail::symbol_index index) const
        -> const uint8_t* = 0;
    ...
    virtual void recode_symbol(uint8_t* symbol, uint8_t* coefficients,
...
{
    using stack_type = kodo::detail::block::stack_decoder<Math>;
    ...
    auto coefficients_data(detail::symbol_index index) const
        -> const uint8_t* override
    {
        return m_stack.coefficients_data(index);
    }
...
void decoder::configure(uint64_t symbols, std::size_t symbol_bytes)
{
    assert(m_impl);
...
auto decoder::coefficients_data(uint64_t index) const -> const uint8_t*
{
    assert(m_impl);
    return m_impl->coefficients_data(detail::symbol_index{index});
}
