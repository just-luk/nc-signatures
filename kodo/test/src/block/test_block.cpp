                               ...
                               std::size_t symbol_bytes)
{
    ...
    encoder.configure(2 * symbols, 2 * symbol_bytes);
    decoder.configure(2 * symbols, 2 * symbol_bytes);
...
{
    {
        ...
        run_test_basic_api(kodo::finite_field::binary, symbols, symbol_bytes);
    }
...
TEST(test_block, test_basic_api)
{
    test_basic_api(1, 1600);
    test_basic_api(2, 1600);
    test_basic_api(32, 1600);

    std::size_t symbols = helper::random_symbols(1, 100);
    std::size_t symbol_bytes = helper::random_symbol_bytes();
    test_basic_api(symbols, symbol_bytes);
}
...
static void test_systematic(std::size_t symbols, std::size_t symbol_bytes)
{
    {
        SCOPED_TRACE(testing::Message() << "field = binary");
        run_test_systematic(kodo::finite_field::binary, symbols, symbol_bytes);
    }
...
/// Tests the basic API functionality: basic encoding and decoding
TEST(test_block, test_systematic)
{
    test_systematic(1, 1600);
    test_systematic(2, 1600);
    test_systematic(32, 1600);

    std::size_t symbols = helper::random_symbols(1, 100);
    std::size_t symbol_bytes = helper::random_symbol_bytes();
    test_systematic(symbols, symbol_bytes);
}
                                ...
                                std::size_t symbol_bytes)
{
    ...
    while (!decoder.is_complete())
    {
        ...
        if (encoder.rank() == encoder.symbols())
            ++received_symbols;
        ASSERT_LT(received_symbols, symbols + 10);
    }
...
/// Tests the basic API functionality: basic encoding and decoding
TEST(test_block, test_on_the_fly)
{
    test_on_the_fly(1, 1600);
    test_on_the_fly(2, 1600);
    test_on_the_fly(32, 1600);

    std::size_t symbols = helper::random_symbols(1, 100);
    std::size_t symbol_bytes = helper::random_symbol_bytes();
    test_on_the_fly(symbols, symbol_bytes);
}
...
/// Tests the basic API functionality: basic encoding and decoding
TEST(test_block, test_recoding)
{
    test_recoding(1, 1600);
    test_recoding(2, 1600);
    test_recoding(32, 1600);

    std::size_t symbols = helper::random_symbols(1, 100);
    std::size_t symbol_bytes = helper::random_symbol_bytes();
    test_recoding(symbols, symbol_bytes);
}
