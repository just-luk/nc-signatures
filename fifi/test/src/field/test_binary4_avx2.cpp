...
static constexpr fifi::field::binary4_avx2 math(0x13);

TEST(test_binary4_avx2, enabled)
{
    cpuid::cpuinfo cpuid;
    EXPECT_EQ(cpuid.has_avx2(), fifi::detail::is_acceleration_enabled(math));
}

TEST(test_binary4_avx2, name)
{
    EXPECT_STREQ("binary4_avx2", fifi::field::binary4_avx2::name);
    EXPECT_STREQ("binary4_avx2", math.name) << math.name;
}

TEST(test_binary4_avx2, vector_multiply_add_into)
{
...
TEST(test_binary4_avx2, vector_multiply_add_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_avx2, vector_multiply_into)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_avx2, vector_multiply_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_avx2, vector_dot_product)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_avx2, vector_dot_product_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
