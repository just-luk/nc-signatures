...
#include <fifi/field/binary4_neon.hpp>
...
static constexpr fifi::field::binary4_neon math(0x13);

TEST(test_binary4_neon, enabled)
{
    cpuid::cpuinfo cpuid;
    EXPECT_EQ(cpuid.has_neon(), fifi::detail::is_acceleration_enabled(math));
}

TEST(test_binary4_neon, name)
{
    EXPECT_STREQ("binary4_neon", fifi::field::binary4_neon::name);
    EXPECT_STREQ("binary4_neon", math.name) << math.name;
}

TEST(test_binary4_neon, vector_multiply_into)
{
...
TEST(test_binary4_neon, vector_multiply_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_neon, vector_multiply_add_into)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_neon, vector_multiply_add_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_neon, vector_dot_product)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_neon, vector_dot_product_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
