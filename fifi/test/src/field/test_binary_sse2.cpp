...
static constexpr fifi::field::binary_sse2 math;

TEST(test_binary_sse2, enabled)
{
    cpuid::cpuinfo cpuid;
    EXPECT_EQ(cpuid.has_sse2(), fifi::detail::is_acceleration_enabled(math));
}

TEST(test_binary_sse2, name)
{
    EXPECT_STREQ("binary_sse2", fifi::field::binary_sse2::name);
    EXPECT_STREQ("binary_sse2", math.name) << math.name;
}

TEST(test_binary_sse2, vector_add_into)
{
...
TEST(test_binary_sse2, vector_add_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
