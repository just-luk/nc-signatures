...
static constexpr fifi::field::binary4_ssse3 math(0x13);

TEST(test_binary4_ssse3, enabled)
{
    cpuid::cpuinfo cpuid;
    EXPECT_EQ(cpuid.has_ssse3(), fifi::detail::is_acceleration_enabled(math));
}

TEST(test_binary4_ssse3, name)
{
    EXPECT_STREQ("binary4_ssse3", fifi::field::binary4_ssse3::name);
    EXPECT_STREQ("binary4_ssse3", math.name) << math.name;
}

TEST(test_binary4_ssse3, vector_multiply_add_into)
{
...
TEST(test_binary4_ssse3, vector_multiply_add_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_ssse3, vector_multiply_into)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_ssse3, vector_multiply_into_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_ssse3, vector_dot_product)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
...
TEST(test_binary4_ssse3, vector_dot_product_below_granularity)
{
    if (!fifi::detail::is_acceleration_enabled(math))
    {
        return;
