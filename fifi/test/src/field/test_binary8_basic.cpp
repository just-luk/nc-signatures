...
static constexpr fifi::field::binary8_basic math(0x11D);

TEST(test_binary8_basic, name)
{
    EXPECT_STREQ("binary8_basic", fifi::field::binary8_basic::name);
    EXPECT_STREQ("binary8_basic", math.name) << math.name;
}

TEST(test_binary8_basic, add)
{
    helper::test_add(math);
}

TEST(test_binary8_basic, subtract)
{
    helper::test_subtract(math);
}

TEST(test_binary8_basic, multiply)
{
    helper::test_multiply(math);
}

TEST(test_binary8_basic, divide)
{
    helper::test_divide(math);
}

TEST(test_binary8_basic, invert)
{
    helper::test_invert(math);
}

TEST(test_binary8_basic, vector_multiply_into)
{
    helper::test_vector_multiply_into(math, helper::random_vector_size(1U));
}

TEST(test_binary8_basic, vector_multiply_into_0)
{
    helper::test_vector_multiply_into(math, helper::random_vector_size(1U), 0U);
}

TEST(test_binary8_basic, vector_multiply_into_1)
{
    helper::test_vector_multiply_into(math, helper::random_vector_size(1U), 1U);
}

TEST(test_binary8_basic, vector_multiply_add_into)
{
    helper::test_vector_multiply_add_into(math, helper::random_vector_size(1U));
}

TEST(test_binary8_basic, vector_multiply_add_into_0)
{
    helper::test_vector_multiply_add_into(math, helper::random_vector_size(1U),
                                          0U);
}

TEST(test_binary8_basic, vector_multiply_add_into_1)
{
    helper::test_vector_multiply_add_into(math, helper::random_vector_size(1U),
                                          1U);
}

TEST(test_binary8_basic, vector_dot_product)
{
