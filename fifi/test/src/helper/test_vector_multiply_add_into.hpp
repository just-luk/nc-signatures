...
namespace helper
{
// Test helper calculating the vector multiply add. First using the provided
...
template <class Math>
inline void test_vector_multiply_add_into(const Math& math, std::size_t size,
                                          uint32_t constant)
{
    ...
    for (std::size_t i = 0; i < elements; ++i)
    {
        ...
        uint32_t expected = fifi::online::add(
...
inline void test_vector_multiply_add_into(const Math& math, std::size_t size)
{
    ...
    test_vector_multiply_add_into(math, size, 0U);

    // Special case test with 1
    test_vector_multiply_add_into(math, size, 1U);

    // Test with random value
    uint32_t constant = random_value(field);
    test_vector_multiply_add_into(math, size, constant);
}
}
