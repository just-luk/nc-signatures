...
namespace field
{
/// Finite field operations in the binary extension field 2^8.
class binary8_math
{
public:
    /// The backend used
    using backend_type = binary8_backend;

    /// The finite field
    using field_type = binary8;
...
public:
    ...
    /// Constructor
    binary8_math(const binary8_backend* backend);
    ...
    void set_backend(const binary8_backend* backend);
    void set_backend(const binary8_backend* backend,
                     const detail::cpu_support& cpu);
...
private:
    ...
    const binary8_backend* m_backend;
    ...
    /// Binary NEON acceleration
    detail::enabled_ptr<binary_neon> m_binary_neon;

    /// Binary basic acceleration
    detail::enabled_ptr<binary_basic> m_binary_basic;

    /// Binary4 basic acceleration
    detail::enabled_ptr<binary8_basic> m_binary8_basic;

    /// Binary4 SSSE3 acceleration
    detail::enabled_ptr<binary8_ssse3> m_binary8_ssse3;

    /// Binary AVX2 acceleration
    detail::enabled_ptr<binary8_avx2> m_binary8_avx2;

    /// Binary4 NEON acceleration
    detail::enabled_ptr<binary8_neon> m_binary8_neon;
