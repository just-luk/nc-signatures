...
static constexpr field::binary8_backend default_binary8{field::binary8()};

/// Initialize cpu support
static const detail::cpu_support cpu;

binary8_math::binary8_math() : binary8_math::binary8_math(&default_binary8)
{
}

binary8_math::binary8_math(const binary8_backend* backend) :
    binary8_math::binary8_math(backend, cpu)
{
}
...
void binary8_math::set_backend(const binary8_backend* backend)
{
    set_backend(backend, cpu);
}
                               ...
                               const detail::cpu_support& cpu)
{
    ...
    m_binary8_basic = {&m_backend->m_binary8_basic, true};
    m_binary8_ssse3 = {&m_backend->m_binary8_ssse3, cpu.ssse3};
    m_binary8_avx2 = {&m_backend->m_binary8_avx2, cpu.avx2};
    m_binary8_neon = {&m_backend->m_binary8_neon, cpu.neon};

    auto check_prime = [](auto a, auto prime)
    { return !a.is_enabled() || a->field_info().prime() == prime; };

    assert(check_prime(m_binary8_basic, field_info().prime()));
    assert(check_prime(m_binary8_ssse3, field_info().prime()));
    assert(check_prime(m_binary8_avx2, field_info().prime()));
    assert(check_prime(m_binary8_neon, field_info().prime()));

    // Silence warnings if compiling without asserts
    (void)check_prime;
}

finite_field binary8_math::field() const
{
    return fifi::finite_field::binary8;
}

binary8 binary8_math::field_info() const
{
    assert(m_backend != nullptr);
    return m_backend->m_field;
}

uint32_t binary8_math::add(uint32_t a, uint32_t b) const
{
    m_field_metrics.m_no_simd += 1;
    return m_binary8_basic->add(a, b);
}

uint32_t binary8_math::subtract(uint32_t a, uint32_t b) const
{
    m_field_metrics.m_no_simd += 1;
    return m_binary8_basic->subtract(a, b);
}

uint32_t binary8_math::multiply(uint32_t a, uint32_t b) const
{
    m_field_metrics.m_no_simd += 1;
    return m_binary8_basic->multiply(a, b);
}

uint32_t binary8_math::divide(uint32_t a, uint32_t b) const
{
    m_field_metrics.m_no_simd += 1;
    return m_binary8_basic->divide(a, b);
}

uint32_t binary8_math::invert(uint32_t a) const
{
    m_field_metrics.m_no_simd += 1;
    return m_binary8_basic->invert(a);
}

void binary8_math::vector_add_into(uint8_t* x, const uint8_t* y,
...
void binary8_math::vector_subtract_into(uint8_t* x, const uint8_t* y,
                                        std::size_t size) const
{
    vector_add_into(x, y, size);
}

void binary8_math::vector_multiply_into(uint8_t* x, uint32_t constant,
                                        std::size_t size) const
{
...
#if defined(PLATFORM_X86)
    vector_multiply_into(m_binary8_avx2, m_field_metrics.m_avx2);
    vector_multiply_into(m_binary8_ssse3, m_field_metrics.m_ssse3);
#elif defined(PLATFORM_NEON)
    vector_multiply_into(m_binary8_neon, m_field_metrics.m_neon);
#endif
    vector_multiply_into(m_binary8_basic, m_field_metrics.m_no_simd);

    assert(size == 0);
}

void binary8_math::vector_multiply_add_into(uint8_t* x, const uint8_t* y,
                                            uint32_t constant,
                                            std::size_t size) const
{
...
#if defined(PLATFORM_X86)
    vector_multiply_add_into(m_binary8_avx2, m_field_metrics.m_avx2);
    vector_multiply_add_into(m_binary8_ssse3, m_field_metrics.m_ssse3);
#elif defined(PLATFORM_NEON)
    vector_multiply_add_into(m_binary8_neon, m_field_metrics.m_neon);
#endif
    vector_multiply_add_into(m_binary8_basic, m_field_metrics.m_no_simd);

    assert(size == 0);
}

void binary8_math::vector_multiply_subtract_into(uint8_t* x, const uint8_t* y,
                                                 uint32_t constant,
                                                 std::size_t size) const
{
...
void binary8_math::vector_dot_product(uint8_t** x, const uint8_t** y,
...
#if defined(PLATFORM_X86)
    vector_dot_product(m_binary8_avx2, m_field_metrics.m_avx2);
    vector_dot_product(m_binary8_ssse3, m_field_metrics.m_ssse3);
#elif defined(PLATFORM_NEON)
    vector_dot_product(m_binary8_neon, m_field_metrics.m_neon);
#endif
    vector_dot_product(m_binary8_basic, m_field_metrics.m_no_simd);

    assert(size == 0);
}

std::size_t binary8_math::total_processed() const
{
    return detail::get_total_field_metrics(m_metrics);
}

auto binary8_math::metric_name(field_metric id) const -> std::string
{
    return m_metrics.metric_name(static_cast<uint64_t>(id));
}

auto binary8_math::metric_value(field_metric id) const -> uint64_t
{
    return m_metrics.metric_value(static_cast<uint64_t>(id));
}

void binary8_math::copy_metrics_storage(uint8_t* data) const
{
    m_metrics.copy_storage(data);
}

auto binary8_math::metrics_storage_bytes() const -> std::size_t
{
    return m_metrics.storage_bytes();
}

auto binary8_math::metrics_to_json() const -> std::string
{
    return m_metrics.to_json();
}

void binary8_math::reset_metrics()
{
    m_metrics.reset_metrics();
}

void binary8_math::reset_metric(field_metric id)
{
    m_metrics.reset_metric(static_cast<uint64_t>(id));
}

auto binary8_math::metrics() const -> std::size_t
{
    return m_metrics.max_metrics();
}

auto binary8_math::metric_id(std::size_t index) const -> field_metric
{
