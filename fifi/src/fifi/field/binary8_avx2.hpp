...
public:
    ...
    static constexpr char name[] = "binary8_avx2";

public:
#ifdef FIFI_PLATFORM_X86
    /// Constructor
    constexpr binary8_avx2(binary8 field) noexcept :
        m_field(field), m_low(), m_high()
    {
        ...
        for (uint32_t i = 0; i < binary8::order; ++i)
        {
            for (uint32_t j = 0; j < 16; ++j)
            {
                ...
                m_low[i][j + 16] = result_low;

                m_high[i][j] = result_high;
                m_high[i][j + 16] = result_high;
            }
        }
    }
#else
    /// Constructor
    constexpr binary8_avx2(binary8 field) noexcept : m_field(field)
    {
    }
#endif
    ...
    /// @copydoc acceleration::vector_dot_product()
    std::size_t vector1_dot_product(uint8_t** x, const uint8_t** y,
                                    const uint8_t** constants, std::size_t size,
                                    std::size_t x_vectors,
                                    std::size_t y_vectors) const;

    /// @copydoc acceleration::vector_dot_product()
    std::size_t vector2_dot_product(uint8_t** x, const uint8_t** y,
                                    const uint8_t** constants, std::size_t size,
                                    std::size_t x_vectors,
                                    std::size_t y_vectors) const;

    /// @copydoc acceleration::vector_dot_product()
    std::size_t vector3_dot_product(uint8_t** x, const uint8_t** y,
                                    const uint8_t** constants, std::size_t size,
                                    std::size_t x_vectors,
                                    std::size_t y_vectors) const;

    /// @copydoc acceleration::vector_dot_product()
    std::size_t vector4_dot_product(uint8_t** x, const uint8_t** y,
                                    const uint8_t** constants, std::size_t size,
                                    std::size_t x_vectors,
                                    std::size_t y_vectors) const;
...
private:
    ...
    detail::table_data<uint8_t, 256, 32> m_low;

    /// Table data for the high 4 bit field elements of a byte
    detail::table_data<uint8_t, 256, 32> m_high;
#endif
};
