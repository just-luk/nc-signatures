...
struct binary8
{
    ...
    /// @copydoc field::field
    static constexpr finite_field field = finite_field::binary8;

    /// @copydoc field::order
    static constexpr order_type order = 256U;

    /// @copydoc field::max_value
    static constexpr order_type max_value = 255U;

    /// @copydoc field::degree
    static constexpr degree_type degree = 8U;

    /// @copydoc field::granularity
    static constexpr uint8_t granularity = sizeof(value_type);

    /// Default construct the binary8 field (default prime)
    constexpr binary8() : m_prime(0x11D)
    {
    }

    /// Construct from a prime (in decimal representation)
    constexpr binary8(prime_type prime) : m_prime(prime)
    {
    }
