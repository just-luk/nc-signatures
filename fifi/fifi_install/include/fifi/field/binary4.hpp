...
struct binary4
{
    ...
    /// @copydoc field::field
    static constexpr finite_field field = finite_field::binary4;

    /// @copydoc field::order
    static constexpr order_type order = 16U;

    /// @copydoc field::max_value
    static constexpr order_type max_value = 15U;

    /// @copydoc field::degree
    static constexpr degree_type degree = 4U;

    /// @copydoc field::granularity
    static constexpr uint8_t granularity = sizeof(value_type);

    /// Default construct the binary4 field (default prime)
    constexpr binary4() : m_prime(0x13)
    {
    }

    /// Construct from a prime (in decimal representation)
    constexpr binary4(prime_type prime) : m_prime(prime)
    {
    }
