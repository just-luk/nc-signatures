...
public:
    /// The backend used
    using backend_type = binary_backend;

    /// The finite field
    using field_type = binary;
...
public:
    ...
    /// Constructor
    binary_math(const binary_backend* backend);

    /// Constructor
    binary_math(const binary_backend* backend, const detail::cpu_support& cpu);

    /// Change the backend used
    void set_backend(const binary_backend* backend);
    void set_backend(const binary_backend* backend,
...
private:
    ...
    const binary_backend* m_backend;
